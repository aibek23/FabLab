 ...
 const AuthPage = ({ role, setRole , auth}) => {
  ...
  const loginHandler = async () => {
    ...
    try {
      ...
      setRole(data.role);
      checkbox&&localStorage.setItem("localST", JSON.stringify({
        token: data.token,
        password: data.password,
        username: data.username,
        userEmail: data.userEmail,
              ...
              <div className={styles.tab_content + " tabs mt-5"}>{tabPanes.activebtn1 ?
                ...
                <div role="tabpanel" className="tab-pane  " id="Section2"  >
                  <form className={styles.form_horizontal}>
                    ...
                    <select className="form-select form-select-md" name="role" value={form.role} onChange={changeHandler}>
                      <option defaultValue>выберите роль пользователя</option>
                      <option value="user">студент</option>
                      <option value="manager">учитель</option>
